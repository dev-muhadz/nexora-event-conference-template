...
function tabs(){const b=document.querySelectorAll(".tabs button"),p=document.querySelectorAll(".schedule");b.forEach(x=>x.onclick=()=>{b.forEach(y=>y.classList.remove("active"));p.forEach(y=>y.classList.remove("active"));x.classList.add("active");document.querySelector(`[data-panel="${x.dataset.day}"]`).classList.add("active")})}
...
function speakers(){
 ...
 document.querySelectorAll(".speakers button").forEach(c=>c.onclick=()=>{let d=data[c.dataset.speaker];document.getElementById("name").textContent=d[0];document.getElementById("role").textContent=d[1];document.getElementById("bio").textContent=d[2];modal.classList.add("open")});
...
function nav(){let m=document.getElementById("menu"),n=document.getElementById("nav");m.onclick=()=>{let o=n.classList.toggle("open");m.textContent=o?"×":"☰";m.setAttribute("aria-expanded",o)};n.querySelectorAll("a").forEach(a=>a.onclick=()=>n.classList.remove("open"))}
